...
public class SelectMediaAlgorithm {
	...
	public static void randomSelect( Connection  x,Object objOutputInterface, String [] args  ) throws Exception{
		...
		System.out.println("Se elegierón "+ NumFotos+ " fotos con una duracion de "+DuracionFoto +" por foto; y "+NumVideos +" Videos con una duracion de "+DuracionVideo);
		...
		while (rs.next()){
			...
			{
				double inicio=0;
				if (rs.getDouble(3)>DuracionVideo) {
					inicio = (double) (Math.random() * (rs.getDouble(3) - DuracionVideo));
				}
				//ojo en SEGUNDOS
